...
!  (in,opt)  mask - If present, use this real array as the data mask.
  ...
  elseif ( size(field,1) == diag%ied-diag%isd +2 ) then
    ishift = 1
!  else
!    call SIS_error(FATAL,"post_data_2d: peculiar size in i-direction")
  endif
  ...
  elseif ( size(field,2) == diag%jed-diag%jsd +2 ) then
    jshift = 1
!  else
!    call SIS_error(FATAL,"post_data_2d: peculiar size in j-direction")
  endif

  if (present(mask)) then
    if ((size(field,1) /= size(mask,1)) .or. &
        (size(field,2) /= size(mask,2))) then
      call SIS_error(FATAL, "post_data_2d: post_data called with a mask "//&
...
!  (in,opt)  mask - If present, use this real array as the data mask.
  ...
  elseif ( size(field,1) == diag%ied-diag%isd +2 ) then
    ishift = 1
!  else
!    call SIS_error(FATAL,"post_data_3d: peculiar size in i-direction")
  endif
  if ( size(field,2) == diag%jed-diag%jsd +1 ) then
    jshift = 0
  elseif ( size(field,2) == diag%jed-diag%jsd +2 ) then
    jshift = 1  
!  else
!    call SIS_error(FATAL,"post_data_3d: peculiar size in j-direction")
  endif
  ...
  if (present(mask)) then
    if ((size(field,1) /= size(mask,1)) .or. &
      ...
      call SIS_error(FATAL, "post_data_3d: post_data called with a mask "//&
                             "that does not match the size of field.")
    endif
  endif
...
function register_SIS_diag_field(module_name, field_name, axes, init_time, &
  ...
  integer :: register_SIS_diag_field
...
!  (in,opt)  tile_count - No clue. (Not used in MOM.)
  ...
  register_SIS_diag_field = register_diag_field_fms(module_name, field_name, axes, &
  ...
  if (is_root_pe() .and. doc_unit > 0) then
    if (register_SIS_diag_field > 0) then
