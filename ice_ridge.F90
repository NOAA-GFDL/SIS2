...
!Niki: The following two were commented out
    ...
    do k=2,km
       if (cn(k)>1.e-10) then
	  thick(k) = hi(k)/cn(k)   ! actual ice thickness; hi is volume here
       else
	  thick(k)=0.0
       end if
    end do
...
! set in namelist: part_par  = -20.   ! this is -1/a*; CICE standard is a* = 0.05
       ...
       do k=0,km
	  rdgtmp(k) = exp(ccn(k)*part_par) * part_pari
       end do
       do k=1,km
	  part_undef(k) = rdgtmp(k-1) - rdgtmp(k)
       end do
       ! ************
       ! *   Aii    *
       ! ************
...
if (Rtot>0.0) then
	...
	!end if                                              !  sea also change of hi_rdg below
	...
	vlev = vlev + frac_hi(kd)             ! diagnosing total ice volume moved due to ridging 
        ...
        hi_rdg(kd) = hi_rdg(kd) &             ! here it is assumed that level and ridged ice
		   - rdg_frac(kd)*frac_hi(kd) !  of a category participate in ridging in equal
		                              !  measure; this also means that ridged ice may
					      !  be ridged again
    ...
    end do   ! kd loop over donating categories
