...
use SIS_error_checking, only : B_chksum, h_chksum, u_chksum, v_chksum
...
!  (in,opt)  haloshift - If present, check halo points out this far.
  ...
  call h_chksum(IST%part_size, trim(mesg)//" IST%part_size", G%HI, haloshift=hs)
  call h_chksum(IST%mH_ice*IG%H_to_kg_m2, trim(mesg)//" IST%mH_ice", G%HI, haloshift=hs)
  do k=1,IG%NkIce
    write(k_str1,'(I8)') k
    k_str = "("//trim(adjustl(k_str1))//")"
    call h_chksum(IST%enth_ice(:,:,:,k), trim(mesg)//" IST%enth_ice("//trim(k_str), G%HI, haloshift=hs)
    call h_chksum(IST%sal_ice(:,:,:,k), trim(mesg)//" IST%sal_ice("//trim(k_str), G%HI, haloshift=hs)
  enddo
  call h_chksum(IST%mH_snow*IG%H_to_kg_m2, trim(mesg)//" IST%mH_snow", G%HI, haloshift=hs)
  call h_chksum(IST%enth_snow(:,:,:,1), trim(mesg)//" IST%enth_snow", G%HI, haloshift=hs)
  if (associated(IST%u_ice_B)) call B_chksum(IST%u_ice_B, mesg//" IST%u_ice_B", G%HI, haloshift=hs)
  if (associated(IST%v_ice_B)) call B_chksum(IST%v_ice_B, mesg//" IST%v_ice_B", G%HI, haloshift=hs)
  call check_redundant_B(mesg//" IST%u/v_ice", IST%u_ice_B, IST%v_ice_B, G)
  if (IST%Cgrid_dyn) then
    call u_chksum(IST%u_ice_C, mesg//" IST%u_ice_C", G%HI, haloshift=hs)
    call v_chksum(IST%v_ice_C, mesg//" IST%v_ice_C", G%HI, haloshift=hs)
